...
1. **Slideshow Gesture Controls**: Configurable single-finger swipe gestures for switching into the Slideshow view, cycling to the next photo, returning to the Home/Clock view, displaying photo capture metadata badges, and managing smart inactivity timeouts.
...
- **Features**:
  - **Clock ➔ Slideshow**: Swipe left (`left_1`) from the clock screen opens the Slideshow.
  ...
  - **Smart Timeout**: Automatically returns to Home after an inactivity period (default: 5 min) when unpaused. Pausing the slideshow suspends the timeout indefinitely.
